...
echo "   Command: ./sidekick get pr-comments -j <PR_URL>"
echo "   Output: Clean JSON without any decorative text"
echo ""

echo "2. Verbose logging (-v flag):"
echo "   Command: ./sidekick get pr-comments -v <PR_URL>"
...
echo "   Command: ./sidekick get pr-comments -o output.json <PR_URL>"
echo ""

echo "Example - JSON-only mode with real PR:"
echo "----------------------------------------"
./sidekick get pr-comments -j https://github.com/aiconsultancy/indra/pull/985 2>/dev/null | jq '{
...
echo "-------------------------------"
./sidekick get pr-comments -j https://github.com/fake/repo/pull/99999 2>/dev/null | jq '{
  valid: .pr_info.valid,
